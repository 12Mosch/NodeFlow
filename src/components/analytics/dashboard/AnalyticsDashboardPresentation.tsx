...
import { toDifficultyBucketLabel } from './useAnalyticsInteractions'
import type { AnalyticsDashboardInteractions } from './useAnalyticsInteractions'
import type {
  AnalyticsDashboardDataValue,
  DerivedAnalytics,
  DifficultyBucketCardItem,
  DifficultyBucketCardsResult,
  RetentionSeries,
  RetentionSeriesKey,
} from './types'
import type { DifficultyBucketLabel } from '../../../../shared/analytics-buckets'
import type { HourlyPerformance } from '../suggestion-helpers'
...
interface AnalyticsDashboardPresentationProps {
  data: AnalyticsDashboardDataValue
  derived: DerivedAnalytics
  retentionSeries: RetentionSeries
  offsetMinutes: number
  interactions: AnalyticsDashboardInteractions
  difficultyBucketCards: DifficultyBucketCardsResult | undefined
  isDifficultyCardsPending: boolean
  onBack: () => void
}

export function AnalyticsDashboardPresentation({
  data,
  derived,
  retentionSeries,
  offsetMinutes,
  interactions,
  difficultyBucketCards,
  isDifficultyCardsPending,
  onBack,
}: AnalyticsDashboardPresentationProps) {
  const {
  ...
  const recommendationAction =
    recommendationIndicator === 'low-data' ? (
      <Badge variant="secondary" className="text-[10px]">
        Low data confidence
      </Badge>
    ) : recommendationIndicator === 'latest-logs' ? (
      <Badge variant="secondary" className="text-[10px]">
            ...
            <AnalyticsCard className="px-6 lg:col-span-2">
                ...
                legend={
                  <>
                    <LegendItem
                      ...
                      onToggle={() => toggleSelectedRetentionSeries('seven')}
                    />
                    <LegendItem
                      label="30-day"
                      ...
                      onPointerLeave={() => setHoveredRetentionSeries(null)}
                      onToggle={() => toggleSelectedRetentionSeries('thirty')}
                    />
                    <LegendItem
                      label="90-day"
                      ...
                      onPointerLeave={() => setHoveredRetentionSeries(null)}
                      onToggle={() => toggleSelectedRetentionSeries('ninety')}
                    />
                    <LegendItem
              ...
              >
                <RetentionChart
                  ...
                  onSeriesSelectChange={toggleSelectedRetentionSeries}
                />
              </ChartFrame>
            </AnalyticsCard>
            ...
            <AnalyticsCard className="px-6">
                ...
                {data.difficulty.total === 0 ? (
                  ...
                  <>
                    <DonutChart
                      ...
                      onSelectLabel={toggleSelectedDifficultyBucket}
                      ...
                      {data.difficulty.buckets.map((bucket, index) => (
                        <button
                          ...
                          onClick={() => {
                            const label = toDifficultyBucketLabel(bucket.label)
                            if (!label) return
                            toggleSelectedDifficultyBucket(label)
                          }}
                          onMouseEnter={() => {
                            const label = toDifficultyBucketLabel(bucket.label)
                            if (!label) return
              ...
              {data.difficulty.total > 0 ? (
                <DifficultyBucketCardList
                  ...
                  onClear={clearSelectedDifficultyBucket}
                />
              ) : null}
            </AnalyticsCard>
          </div>
...
function RetentionChart({
  ...
  onSeriesSelectChange: (series: RetentionSeriesKey) => void
}) {
  const chartId = useId()
      ...
      {seriesConfigs.map((seriesConfig) => {
        ...
        return (
          <path
            ...
            onClick={() => onSeriesSelectChange(seriesConfig.key)}
            onKeyDown={(event) => {
              if (event.key === 'Enter' || event.key === ' ') {
                event.preventDefault()
                onSeriesSelectChange(seriesConfig.key)
              }
            }}
          />
        )
...
function HourlyChart({
  ...
  const tickLabels =
    orderedData.length === 0
      ...
      : Array.from({ length: TICK_SEGMENTS + 1 }, (_, i) => {
          // The 5th tick repeats midnight so the 24-hour axis visually wraps.
          const raw =
            i < TICK_SEGMENTS
              ? Math.round((i / TICK_SEGMENTS) * (orderedData.length - 1))
              : 0
