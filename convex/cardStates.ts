    ...
    return await (async () => {
      ...
      for (const block of enabledBlockById.values()) {
        ...
        if (outlineAncestorNodeIds && outlineAncestorNodeIds.length > 0) {
          outlineAncestorNodeIdsByBlockId.set(block._id, outlineAncestorNodeIds)
      ...
      await Promise.all(
        Array.from(ancestorNodeIds).map(async (nodeId) => {
          const ancestorBlock = await ctx.db
            ...
            .unique()
          if (!ancestorBlock) return
          const normalizedText = normalizeTextContent(ancestorBlock.textContent)
          if (!normalizedText) return
...
export const getAnalyticsDashboard = query({
  ...
  handler: async (ctx, args) => {
    return await (async () => {
      ...
      const dailyRetention = Array.from({ length: rangeDays }, (_, index) => ({
      ...
      const peakCandidates = hourlyPerformance.filter((hour) => hour.total > 0)
      const peakFiltered = peakCandidates.filter((hour) => hour.total >= 5)
      const peakSource = peakFiltered.length > 0 ? peakFiltered : peakCandidates
      ...
      const totalDueNext30 = dueCounts.reduce((sum, day) => sum + day.count, 0)
      return {
        retention: {
